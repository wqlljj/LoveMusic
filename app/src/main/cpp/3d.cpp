...
const int INTERFACE_UPDATETIME_3D = 50;      // 50ms update for interface
    ...
    */
    do {
        ...
        // ==========================================================================================
        {
            ...
            if (listenerflag) {
                listenerpos.x =
                        (float) sin(t * 0.05f) * 24.0f * DISTANCEFACTOR; // left right pingpong
            }

            // ********* NOTE ******* READ NEXT COMMENT!!!!!
            // vel = how far we moved last FRAME (m/f), then time compensate it to SECONDS (m/s).
            vel.x = (listenerpos.x - lastpos.x) * (1000 / INTERFACE_UPDATETIME_3D);
            vel.y = (listenerpos.y - lastpos.y) * (1000 / INTERFACE_UPDATETIME_3D);
            vel.z = (listenerpos.z - lastpos.z) * (1000 / INTERFACE_UPDATETIME_3D);
            ...
            t += (30 * (1.0f /
                        (float) INTERFACE_UPDATETIME_3D));    // t is just a time value .. it increments in 30m/s steps in this example
        }
        ...
        Common_Draw("Press %s or %s to move listener in still mode", Common_BtnStr(BTN_LEFT),
                    Common_BtnStr(BTN_RIGHT));
        Common_Draw("Press %s to toggle listener auto movement", Common_BtnStr(BTN_MORE));
        Common_Draw("Press %s to quit", Common_BtnStr(BTN_QUIT));
        Common_Draw("");
        Common_Draw(s);

        Common_Sleep(INTERFACE_UPDATETIME_3D - 1);
